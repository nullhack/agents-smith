Feature: CLI Entrypoint
  ...
  The application exposes a command-line interface via `python -m smith`.
  ...
  implementation lives in `smith/__main__.py` with no new dependencies — both
  ...
  - All production code lives in `smith/__main__.py` only — no new files.
  ...
  Rule: Help output
    ...
    I want to run `python -m smith --help` and see the app name, tagline, and available options
  ...
  Rule: Version output
    As a developer using the template
    I want to run `python -m smith --version` and see the current version
    ...
    Example: Version string matches package metadata at runtime
      Given the application package is installed
      When the user runs `python -m smith --version`
      Then the version in the output matches `importlib.metadata.version("smith")`
